    ...
    readme.write_text(
        "\n".join(
            [
                ...
                "```",
                "",
                "On a clean target, the installer copies the bundled project to `~/UAVDetection` on macOS/Linux or `%USERPROFILE%\\UAVDetection` on Windows by default, creates `.venv`, installs Python dependencies from the copied `wheelhouse`, initializes `data_store`, creates or reuses HTTPS certificates, installs an automatic-start service, and updates common browser home/start pages to the local server URL.",
                "",
                "On an existing Windows install, run the same `install_offline.ps1` or `install_offline.cmd` from this bundle. It updates code/support files while preserving the installed `data_store/`. The only `data_store` file intentionally refreshed is `data_store/system_config/cameras.yaml`; the previous camera file is backed up as `cameras.yaml.backup_YYYYMMDD_HHMMSS`. Use `--no-camera-config-update` to skip that refresh. Use `--force` only for a destructive full replace, including local data.",
                "",
                "Default local URL:",
                "",
                "```text",
                ...
                "./install_offline.sh --install-dir ~/UAVDetection --force  # destructive full replace",
